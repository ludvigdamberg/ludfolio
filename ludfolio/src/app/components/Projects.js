...

        {
          Loaded ?
            projects.map((project) =>
              <Link href={`/project/${project.slug}`} key={project._id} className='  border border-neutral-950 dark:border-neutral-200 rounded-xl p-3 md:grid md:grid-cols-3 flex flex-col hover:scale-[102%] ease-in-out duration-150'>
                <div className='md:col-span-2 flex flex-col '>
                  ...
                  <div className='  mt-auto mb-2 md:flex hidden text-xs '>{project.tech.map((t, index) => <p key={index} className='px-1 py-1 bg-neutral-300 dark:bg-neutral-700 rounded-lg mr-1'>{t} </p>)}</div>
                </div>
